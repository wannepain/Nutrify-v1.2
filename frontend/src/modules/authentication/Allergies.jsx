...
function Allergies(props) {
    ...
    useEffect(() => {
        ...
        const savedValues = JSON.parse(localStorage.getItem("allergies"));
        if (savedValues) {
            setAllergens(savedValues);
            if (typeof props.obj === 'function') {
                props.obj({ allergens: savedValues });
            }
        }
    }, []);
    ...
    function handleAdd() {
        if (allergens.includes(userInput)) {
            ...
            const updatedAllergens = [...allergens, userInput.toLowerCase()];
            setAllergens(updatedAllergens);
            if (typeof props.obj === 'function') {
                localStorage.setItem("allergies", JSON.stringify(updatedAllergens));
                props.obj({ allergens: updatedAllergens });
            }
            setUserInput('');
        } else {
            if (typeof props.error === 'function') {
    ...
    return (
        <div>
            ...
            <div className={classes.allergenInputContainer}>
                ...
                <button type="button" className={classes.btn} onClick={handleAdd}>
                    <img src="./../../../public/add_recipe_icon.svg" alt="Add" id="addAllergenIcon" />
                </button>
            </div>
